...
<div class={cardBg .. " rounded-lg p-4 border border-gray-700"}>
    ...
    <div class="text-sm space-y-1 text-gray-400">
        <p>📧 {user.email}</p>
        <p>📍 Page: {nav.currentPage}</p>
        <p>🏷️ App: {app.name} v{app.version}</p>
    </div>
...
<article class="max-w-2xl mx-auto">
    ...
    {#if props.author}
        <cite class="text-sm text-gray-500 dark:text-gray-400 mt-1 block">— {props.author}</cite>
    {/if}
</blockquote>`
    }
  ]}
...
local HomeIcon = createIcon("🏠", "text-blue-500")
local UserIcon = createIcon("👤", "text-green-500")
local SettingsIcon = createIcon("⚙️", "text-gray-500 dark:text-gray-400")
</script>
